...
d&&a(".themify-cm-conditions-container:first").addClass("themify-cm-conditions-container-"+b+c).data("item",b+c);e>d&&a(".themify-cm-conditions-container:first").clone().removeClass().addClass("themify-cm-conditions-container themify-admin-lightbox clearfix themify-cm-conditions-container-"+b+c).data("item",b+c).insertAfter(".themify-cm-conditions-container:last");a(".lightbox_container",".themify-cm-conditions-container-"+b+c).html("")}var k=[],m,n=function(){var a=document;return Math.max(Math.max(a.body.scrollHeight,
...
var g=function(){var b=a('.themify-visibility-options[data-type="'+this.dataset.type+'"] .themify-visibility-items-page input');this.checked?b.bind("change",{type:this.dataset.type},h):b.unbind("change",h)},h=function(b){b=a('.themify-visibility-options[data-type="'+b.data.type+'"] input[data-parent="'+this.dataset.slug+'"]');b.length&&b.prop("checked",this.checked).trigger("change")};a.ajax({url:ajaxurl,type:"post",data:{action:"themify_create_inner_page",type:d,selected:f,original_values:b.val()},
beforeSend:function(){e.html('<div class="tb_slider_loader"></div>')},success:function(b){b=JSON.parse(b);e.html(b.html);"page"!==d&&"category"!==d||document.querySelector(".tf_cm_select_sub input[data-type="+d+"]").addEventListener("change",g);c.data("active","on");a("#themify-cm-original-conditions").val(b.original_values)}})}}).on("click",".themify-visibility-pagination a.page-numbers",function(b){b.preventDefault();b=a(this);var c=b.parents(".themify-visibility-options"),d=b.parents(".themify-visibility-items-inner"),
e=a(".themify-visibility-pagination",d),f=parseFloat(a(".themify-visibility-pagination .current",d).text()),g=1;b.hasClass("next")?g=f+1:b.hasClass("prev")?g=f-1:b.hasClass("page-numbers")&&(g=parseFloat(b.text()));var h=a(".themify-visibility-items-inner",c);a.ajax({url:ajaxurl,type:"post",data:{action:"themify_create_page_pagination",current_page:g,num_of_pages:d.data("pages")},beforeSend:function(){a(".tb_slider_loader",c).remove();e.hide();h.append('<div class="tb_slider_loader"></div>')},success:function(b){a(".tb_slider_loader",
c).remove();a(".themify-visibility-items-page-"+g,d).removeClass("is-hidden");e.html(b).show()}})}).on("click",".themify-cm-close, #themify-cm-overlay",function(b){b.preventDefault();var c=a(this).parents(".themify-cm-conditions-container");c.animate({top:n()},800,function(){a("#themify-cm-overlay").hide();c.hide().addClass("is-loaded")});return!1}).on("click",".themify-mc-add-assignment",function(b){p(a("#locations-"+a(this).closest("tr").attr("data-menu")).closest("tr"));return!1}).on("click",".themify-cm-save",
...
k[b]=++c})});a("#themify-cm-about").appendTo("p.button-controls")});
